...
% but loads the video 'robot2.mp4' to the workspace as robot2.
out = importVideo('C:\Users\Anthony\Documents\School\5. Winter 2018\AMATH 582\Data\HW 4\robot2.mp4');

%% Preprocess the Video

% Convert the RGB video to black and white
vid = convert2gray(robot2);
...
% For some reason there was a glitch in this frame that throws the data
% analysis off if we don't throw it out.
vid(:,:,94) = [];
...
% Truncate matrices via SVD. Keep only 50 out of 149 singular values.
k = 50;
U = U(:,1:k);
Sigma = Sigma(1:k,1:k);
V = V(:,1:k);
...
% v = VideoWriter('dmdReconstruct.avi');
% open(v);
% 
